...
public class QueryDslGroupByTest {
  ...
  public void 프로젝션을_사용하여_나라와_함께_팀의_멤버들을_조회_할_수_있다() {
    ...
    // when
    Map<String, List<MemberEntity>> actual =
        memberRepository.findAllWithCountry().stream()
            .collect(
  ...
  private TeamEntity createTeam(final String country) {
    ...
    TeamEntity entity = TeamEntity.builder().name(country + "Team").country(country).build();
