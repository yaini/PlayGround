...
public class JPAPersistenceContextTest {
  ...
  public void 엔티티의_값을_변경하면_UPDATE_쿼리가_실행된다() {
    ...
    SimpleEntity entity = SimpleEntity.builder().id(saved.getId()).name("change name").build();
  ...
  public void 엔티티의_값을_변경하지_않으면_UPDATE_쿼리가_실행되지_않는다() {
    ...
    SimpleEntity entity = SimpleEntity.builder().id(saved.getId()).name(saved.getName()).build();
  ...
  public void saveAll을_실행해도_bulk_INSERT와_UPDATE가_실행되지_않는다() {
    ...
    list.forEach(entity -> entity.setName("changeName"));
  ...
  private SimpleEntity create(String name) {
    SimpleEntity entity = SimpleEntity.builder().name(name).build();
